...
import type { Discount, DiscountSortField, DiscountType } from '@nursery-os/contracts';
import { DISCOUNT_SORT_FIELDS, DISCOUNT_TYPES } from '@nursery-os/contracts';
...
const DEFAULTS: DiscountListQuery = {
  page: 1,
  pageSize: 20,
  search: '',
  type: '',
...
function readQuery(searchParams: URLSearchParams): DiscountListQuery {
  ...
  return {
    ...
    type: DISCOUNT_TYPES.includes(type as DiscountType) ? (type as DiscountType) : DEFAULTS.type,
    isActive: isActive === 'true' ? true : isActive === 'false' ? false : DEFAULTS.isActive,
    sortBy: DISCOUNT_SORT_FIELDS.includes(sortBy as DiscountSortField)
      ? (sortBy as DiscountSortField)
      : DEFAULTS.sortBy,
    sortOrder: sortOrder === 'asc' || sortOrder === 'desc' ? sortOrder : DEFAULTS.sortOrder,
  };
}

function toSearchString(query: DiscountListQuery): string {
...
export interface DiscountListResult {
  ...
  isLoading: boolean;
  error: unknown;
  setQuery: (partial: Partial<DiscountListQuery>) => void;
  refetch: () => void;
}

/** Same URL-driven query/pagination/sort/debounced-search shape as useFeeList - no bespoke logic here. */
export function useDiscountList(): DiscountListResult {
  ...
  const applyQuery = (next: DiscountListQuery) => {
    router.replace(`/dashboard/configuration/discounts${toSearchString(next)}`);
  };

  const setQuery = (partial: Partial<DiscountListQuery>) => {
    const next: DiscountListQuery = {
      ...query,
      ...partial,
...
export function useDiscount(id: string | null): DiscountResult {
...
export interface DiscountSummaryResult {
  total: number;
  activeCount: number;
  isLoading: boolean;
  error: unknown;
...
export function useDiscountSummary(): DiscountSummaryResult {
  const [total, setTotal] = useState(0);
  ...
  useEffect(() => {
    ...
    Promise.all([
      api.discounts.list({ page: 1, pageSize: 1 }),
      api.discounts.list({ page: 1, pageSize: 1, isActive: true }),
    ])
      .then(([all, active]) => {
        if (cancelled) return;
