...
describe('PricingEngineService', () => {
  ...
  describe('fees', () => {
    it('includes mandatory RECURRING PlanFees at live Fee.amount and optional ChildFeeAssignments at snapshotAmount', async () => {
      planFee.findForPlanComposable.mockResolvedValue([
        { isMandatory: true, fee: { name: 'Meals', amount: D(50), type: 'RECURRING' } },
        { isMandatory: false, fee: { name: 'Transport', amount: D(999), type: 'RECURRING' } },
      ] as never);
    ...
    // Round 3 MVP trial blocker: a mandatory ONE_TIME PlanFee (e.g.
    // Registration Fee) was being regenerated on every period's invoice,
    // indefinitely, for as long as the enrollment stayed active - since
    // BillingRun regeneration always wipes and rebuilds every FEE-sourced
    // line from scratch, and InvoiceLineItem has no fee_id to tell "already
    // charged" from "never charged." ONE_TIME mandatory PlanFees are now
    // never auto-generated here; OneTimeChargeService (sourceType
    // ONE_TIME_CHARGE, excluded from that wipe/rebuild) is the intended,
    // already-built mechanism for a charge like this.
    it('excludes a mandatory ONE_TIME PlanFee - it must never be auto-generated by recurring BillingRun', async () => {
      planFee.findForPlanComposable.mockResolvedValue([
        { isMandatory: true, fee: { name: 'Meals', amount: D(50), type: 'RECURRING' } },
        { isMandatory: true, fee: { name: 'Registration Fee', amount: D(500), type: 'ONE_TIME' } },
      ] as never);

      const result = await service.computeChargesForPeriod('tenant-1', 'child-1', '2026-09-01', '2026-09-30');

      const feeLines = result.drafts.filter((d) => d.sourceType === 'FEE');
      expect(feeLines).toEqual([expect.objectContaining({ description: 'Meals', totalAmount: '50' })]);
      expect(feeLines.find((d) => d.description === 'Registration Fee')).toBeUndefined();
    });

    it('leaves tuition unaffected by the RECURRING/ONE_TIME fee filter', async () => {
      planFee.findForPlanComposable.mockResolvedValue([
        { isMandatory: true, fee: { name: 'Registration Fee', amount: D(500), type: 'ONE_TIME' } },
      ] as never);
      ...
      const tuitionLine = result.drafts.find((d) => d.sourceType === 'PLAN_TUITION');
      expect(tuitionLine).toEqual(
        expect.objectContaining({ description: 'Full Time', totalAmount: '1000', planPriceId: 'price-1' }),
      );
    });
  });
  ...
  describe('discounts', () => {
    ...
    it('computes ALL_CHARGES scope against tuition plus fees, BASE_TUITION_ONLY against tuition alone', async () => {
      planFee.findForPlanComposable.mockResolvedValue([
        { isMandatory: true, fee: { name: 'Meals', amount: D(200), type: 'RECURRING' } },
      ] as never);
      childDiscountAssignment.findEffectiveForPeriod.mockResolvedValue([
