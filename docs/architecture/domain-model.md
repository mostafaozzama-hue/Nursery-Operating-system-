...
- **Configuration Engine extension (design only):** the same rule applies without exception — `plans`, `plan_prices`, `fees`, `plan_fees`, `child_fee_assignments`, `discounts`, `child_discount_assignments`, `sibling_discount_tiers`, `waivers`, `holidays`, `billing_runs`, `payment_allocations`, `manual_overrides`, and `credit_notes` each carry their own `tenant_id` and each need their own RLS policy migration when implemented.
