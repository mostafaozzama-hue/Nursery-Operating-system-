...
- **Currency**: `StaffPayroll.currency` already exists per-record (defaulting `USD`); the same pattern needs to extend to Invoice/Payment once their frontend ships, and ideally a tenant-level default currency setting (paralleling the `Tenant.locale` recommendation above) so it isn't re-entered per record.
