...
**Already done, carried forward as-is:** Auth, Children, Guardians, Child-Guardian relationships, Classrooms, Enrollment, Staff, Payroll, **Attendance**, **Billing**, **Payments** — all built and stable per [feature-map.md](./feature-map.md). Billing specifically has further work planned beyond the design-system visual pass above — see the Billing Configuration engine item directly above.
...
- **Billing Configuration engine (advanced)** — discount/sibling-rate rules, waivers, recurring billing automation (monthly tuition auto-generation), late-fee automation. Builds on top of Phase 1's basic Classes → Plans → Fees configuration; Professional doesn't introduce configuration for the first time, it deepens it.
