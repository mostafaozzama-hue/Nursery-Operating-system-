...
- **Where:** (a) Every list endpoint supports `sortBy`/`sortOrder`; `DataTable` (`components/common/data-table.tsx`) gained sortable-header support (`sortKey`/`onSortChange`) when Attendance shipped, and `InvoiceList` (Billing/Payments) wired it too on ship — but Children/Guardians/Classrooms/Staff/Payroll still render plain, unsorted headers despite the capability now existing in the shared component. (b) Every table tracks `createdBy`/`updatedBy`/`createdAt`/`updatedAt`; there is no activity feed anywhere in the product. (c) ~~Invoice/Payment backend modules are fully built with zero frontend~~ — **shipped**, see [feature-map.md](./feature-map.md#billing); Attendance was in this same position before it and has also since shipped.
- **Severity:** High for (a) — it's a whole-product-surface gap on every pre-Attendance/pre-Invoice list page; Medium for (b) — desirable but not currently blocking any workflow.
...
The following are **not** UX debt — they are missing features, already tracked in [feature-map.md](./feature-map.md) and sequenced in [roadmap.md](./roadmap.md), and are excluded here to avoid duplicating those documents: no Teacher App / Parent App (Phase 2-3), no Multi-Branch support, no RTL/Arabic implementation (foundational convention starts now per [design-system.md §8](./design-system.md#8-rtl--internationalization), full localization ships Phase 4), no bulk actions in `DataTable`, no directory-hook scaling beyond 100 records (both tracked in [enterprise-roadmap.md](./enterprise-roadmap.md) as Enterprise-phase architectural work, not current-build debt). (Attendance and Billing/Payments were both previously listed here as backend-complete/frontend-pending — both have since shipped, see [feature-map.md](./feature-map.md#attendance) and [feature-map.md](./feature-map.md#billing).)
