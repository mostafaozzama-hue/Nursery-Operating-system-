...
- **Where:** (a) Every list endpoint supports `sortBy`/`sortOrder`; `DataTable` (`components/common/data-table.tsx`) has no sort UI at all. (b) Every table tracks `createdBy`/`updatedBy`/`createdAt`/`updatedAt`; there is no activity feed anywhere in the product. (c) Attendance and Invoice/Payment backend modules are fully built with zero frontend (tracked as a feature gap in [feature-map.md](./feature-map.md), not repeated here as debt — but the *sort* and *activity* gaps below are genuine debt, since they're partial-capability gaps on modules that otherwise have a frontend).
- **Severity:** High for (a) — it's a whole-product-surface gap on every list page; Medium for (b) — desirable but not currently blocking any workflow.
- **Business impact:** (a) Users cannot reorder any list by the fields that matter to them (e.g. sort Staff by hire date, sort Payroll by effective date) despite the backend already computing the sort — this is pure unrealized value. (b) No "who changed this and when" visibility anywhere, despite the data existing — becomes a real gap once multiple staff share admin access (medium/enterprise tier).
- **Recommended solution:** (a) Wire clickable, sortable column headers into `DataTable` against the existing `sortBy`/`sortOrder` query params — no backend change needed. (b) Build a shared activity-feed component once a concrete use case needs it (Dashboard's "recent activity" widget in [design-system.md §12](./design-system.md#12-dashboard-vision) is the first real consumer).
- **Priority:** (a) Phase 1, part of the `DataTable` upgrade already scoped in [design-system.md §16](./design-system.md#16-prioritized-ux-improvement-backlog) item 1. (b) Phase 1, as a direct dependency of Dashboard v1's "recent activity" widget.
