...
**Configuration Engine — backend complete, frontend not started.** `Plan` is now a first-class entity (price, billing cycle, permitted attendance schedule, discount eligibility, active/inactive status) with the full supporting service layer built: `PlanService`, `PlanPriceService`, `FeeService`, `PlanFeeService`, `DiscountService`, `SiblingDiscountTierService`, `HolidayService` (definitions); `EnrollmentBillingTermsService`, `ChildFeeAssignmentService`, `ChildDiscountAssignmentService`, `WaiverService` (per-child assignment); `PricingEngineService`, `BillingRunService`, `OneTimeChargeService`, `ManualOverrideService`, `CreditNoteService` (computation/orchestration) — see [configuration-engine-backend-services.md](./configuration-engine-backend-services.md) and [roadmap.md](./roadmap.md) for status. **None of this has an admin UI yet** — a child's invoice is still always hand-entered line items today; see [roadmap.md](./roadmap.md)'s "Configuration Engine Frontend" milestone. Two `PricingEngineService` business-rule gaps (no rule for a child with no effective billing terms; no sibling-tier selection rule) remain genuinely unresolved — product decisions, not implementation debt.

- **MVP:** Manual invoice creation and line items ✅; invoice status lifecycle ✅; frontend for creating/viewing invoices ✅; **Billing Configuration engine (basic)** — Classes → Plans → Fees ⚙️ *(backend complete, frontend not started — see [roadmap.md](./roadmap.md) Phase 1)*, so a child's invoice is generated from a configured `Plan` rather than hand-entered line items for the recurring case. Moved up from Professional; configuration-before-operations is now core product identity, not a Professional-only capability.
- **Professional:** Billing Configuration engine (advanced) — discount/sibling-rate rules, waivers, recurring billing automation (monthly tuition auto-generation) ⚙️ *(backend complete via `DiscountService`/`SiblingDiscountTierService`/`WaiverService`/`BillingRunService`, frontend not started)*; late-fee automation ⬜ *(genuinely unbuilt, no backend logic exists)*. Built on top of MVP's basic configuration rather than introducing configuration for the first time.
- **Also newly available (backend only, not previously named in this document):** `OneTimeChargeService` — ad hoc charges against draft or already-issued invoices, audited via `ManualOverride` ⚙️; `CreditNoteService` — money owed back to a guardian without mutating the original invoice ⚙️ (scoped minimally: creation only, no apply/refund/list endpoints yet — no caller needs them). Both frontend ⬜.
...
Recording and reconciling payment against invoices. **Built** (backend + frontend) — `Payment` is guardian-anchored (recorded against the billing-party `Guardian`, not directly against one invoice) and allocated oldest-invoice-first across all of that guardian's children via `PaymentAllocation`, replacing the original strict invoice-1:1 model. A "Record payment" drawer (`Sheet side="right"`, per [design-system.md §5.13](./design-system.md#513-drawers)) supports cash, Vodafone Cash, InstaPay, bank transfer, card, and check as first-class methods, launched from Invoice Detail (submitting against the invoice's own guardian) and reusable as-is for a future Guardian Detail entry point. `GET /invoices/:id/payments` remains a read-only view onto this data, joined through `PaymentAllocation`.

- **MVP:** Manual payment recording against a guardian's account ✅; cash payment support (a first-class method, not an edge case, per [vision.md](./vision.md)) ✅; frontend for recording payments and viewing an invoice's payment history ✅; **guardian payment history and computed available-credit display on Guardian Detail** ⬜ *("Payment Frontend Phase 2" — see [roadmap.md](./roadmap.md), already scoped, not yet started)*.
